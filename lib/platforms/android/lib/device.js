    ...
    exec(cmd, function (error, stdout) {
        if (error !== null) { defer.reject(error); }
            ...
            var out = stdout.toString(),
                proc = out.match(/Processor(.*)/) || out.match(/model name(.*)/),
                archi = out.match(/CPU architecture(.*)/),
                hardware = out.match(/Hardware(.*)/);

            if(proc) proc = proc[0].split(':')[1].trim();
            if(archi) archi = archi[0].split(':')[1].trim();
            if(hardware) hardware = hardware[0].split(':')[1].trim();
...
var list = function () {
    var defer = Q.defer();
    exec(settings.external.adb.name + ' devices -l', function (error, stdout) {
        if (error !== null) defer.reject(error);
