    ...
    platforms.forEach(function(platform) {
        ...
        if(version){
            ...
            if (versions === undefined) {
              print.error('you should upgrade tarifa to > 0.6.0');
            }
...
function extendPlatformURI(platform) {
    var pkg = require(path.join(__dirname, '../platforms', platform.split('@')[0], 'package.json'));
    return format('%s@%s', pkg.uri, platform.indexOf('@') > -1 ? platform.split('@')[1] : pkg.version);
}
...
function addPlatforms (root, platforms, verbose) {
    ...
    return cordova_platform_add(hooks, appPath, platforms.map(extendPlatformURI), opts).then(function () {
        process.chdir(cwd);
    }).then(function () {
