...
function installedPlatforms() {
    ...
    isOnline(function (err, online) {
        if(err) onlineDefer.reject(err);
        else onlineDefer.resolve(online);
    });

    return onlineDefer.promise.then(function (online) {
        if(!online) {
            log.send('warning', 'network unavailable skipping platform check!');
            return platforms.map(mapping);
        }
