...
function destinationPath(appName, destTmpl, natPath, type, platform, version) {
    var xcassets = platform === 'ios' && version.split('.')[0] >= 4,
        innerPath;
    if (xcassets) {
        innerPath = type === 'icons' ? 'Images.xcassets/AppIcon.appiconset' : 'Images.xcassets/LaunchImage.launchimage';
    } else {
        innerPath = type === 'icons' ? 'Resources/icons' : 'Resources/splash';
    }
    return path.join(natPath, destTmpl.replace('{$app_name}', appName).replace('{$path}', innerPath));
}
...
function copyAssets(root, mapping, platform, configuration, type, version) {
    if(!mapping[platform]) return Q.reject('platform not available!');
            ...
            return promise.then(function () {
                if(tuple.src instanceof Array) {
                    ...
                    tuple.dest.forEach(function (dest) {
                        rimraf.sync(destinationPath(localSettings.name, dest, natPath, type, platform, version));
                    });

                    for(var i = 0, l = tuple.src.length; i < l; i++) {
                        var dest = destinationPath(localSettings.name, tuple.dest[i], natPath, type, platform, version),
                    ...
                    if (fs.existsSync(src)) {
                        return mkdirAndCopy(
                            src,
                            destinationPath(localSettings.name, tuple.dest, natPath, type, platform, version)
                        );
                    }
                    else {
...
function resizeImage(file, size, destination, platform, color) {
    var defer = Q.defer(),
        c = color || 'white',
        opts = platform === 'ios' ? format('-background %s -alpha remove', c) : '',
        cmd = format('convert %s -resize %s %s %s', file, size.join('x'), opts, destination);
    execConvert(cmd, defer);
    return defer.promise;
}
...
function generateFromFile(mapping, file, root, platforms, config, color) {
        ...
        return platforms.reduce(function (platformPromise, platform) {
            return platformPromise.then(function () {
                return mapping[platform].reduce(function (promise, tuple) {
                    ...
                    return promise.then(function () {
                        return resizeImage(file, [dim, dim], dest, platform, color);
                    });
                }, Q());
            });
        }, Q());
...
function copyDefaultAndroidNotifAssets (root) {
    var defer = Q.defer(),
        tmplFolder = path.resolve(__dirname, '..', '..', 'template', 'assets', 'android'),
        folder = path.resolve(root, settings.images, 'android', 'default', 'images');
    mkdirp(folder, function (err) {
        if(err) return defer.reject('unable to create folder ' + folder + ' ' + err);
        fs.readdirSync(tmplFolder).filter(function (fileName) {
            return /.*notif.*\.png/.test(fileName);
        }).reduce(function (p, fn) {
            return p.then(function () {
                return qfs.copy(path.resolve(tmplFolder, fn), path.resolve(folder, fn));
            });
        }, Q()).then(function () {
            defer.resolve();
        });
...
module.exports.copyDefaultAndroidNotifAssets = copyDefaultAndroidNotifAssets;
